...
# ==============================================================================
# --- Silakan sesuaikan path berikut dengan lokasi file model .h5 Anda ---
MODEL_PATHS = {
    'Unet_BraTS2020': 'C:/Aulia/Kumpulan Eksperimen/Eksperimen 1/Model/AttnDropUnet_best.h5',
    'Unet_BraTS2021': 'C:/Aulia/Kumpulan Eksperimen/Eksperimen 2/Model/AttnDropUnet_best.h5',
    'Unet_BraTS2020_2021': 'C:/Aulia/Kumpulan Eksperimen/Eksperimen 3/Model/AttnDropUnet_best.h5',
    'AttnDropUnet_BraTS2020': 'C:/Aulia/Kumpulan Eksperimen/Eksperimen 16/Model/Unet_best.h5',
    'AttnDropUnet_BraTS2021': 'C:/Aulia/Kumpulan Eksperimen/Eksperimen 17/Model/Unet_best.h5',
    'AttnDropUnet_BraTS2020_2021': 'C:/Aulia/Kumpulan Eksperimen/Eksperimen 18/Model/AttnDropUnet_best.h5' # Contoh path yang sudah ada
}

# --- Siapkan Custom Objects yang LENGKAP (sama seperti sebelumnya) ---
...
# --- Loop untuk memuat semua model ---
loaded_models = {}
print("\n--- Memuat Semua Model untuk Perbandingan ---")
for model_name, model_path in MODEL_PATHS.items():
    if os.path.exists(model_path):
        try:
            ...
            print(f"❌ Gagal memuat model '{model_name}'. Error: {e}")
    else:
        print(f"⚠️ Model '{model_name}' tidak ditemukan di path: {model_path}. Model ini akan dilewati.")

if not loaded_models:
    print("\n❌ Tidak ada model yang berhasil dimuat. Skrip tidak dapat melanjutkan visualisasi.")
else:
    print(f"\nTotal model yang berhasil dimuat: {len(loaded_models)}")

# ==============================================================================
# BAGIAN 4 (MODIFIKASI): EVALUASI KUANTITATIF MODEL
# ==============================================================================

# --- PILIH MODEL UNTUK EVALUASI KUANTITATIF ---
# Ganti nama string ini dengan salah satu kunci dari MODEL_PATHS Anda di Bagian 3
MODEL_FOR_EVALUATION = 'AttnDropUnet_BraTS2020_2021' 
# =================================================

# Cek apakah model yang dipilih berhasil dimuat
if MODEL_FOR_EVALUATION in loaded_models and len(val_generator) > 0:
    print(f"\n--- Memulai Evaluasi Kuantitatif pada Model: {MODEL_FOR_EVALUATION} ---")
    
    # Ambil model yang spesifik dari dictionary
    model_to_evaluate = loaded_models[MODEL_FOR_EVALUATION]
    ...
    for i in tqdm(range(len(val_generator)), desc=f"Mengevaluasi {MODEL_FOR_EVALUATION}"):
        img_batch, mask_batch = val_generator[i]
        
        # --- PERUBAHAN UTAMA: Gunakan model_to_evaluate ---
        pred_batch = model_to_evaluate.predict(img_batch, verbose=0)
        # --------------------------------------------------
    ...
    print("\n\n===========================================================")
    print(f"--- HASIL EVALUASI RATA-RATA ({MODEL_FOR_EVALUATION}) ---")
    print("===========================================================")
    ...
    print("\n[BAGIAN 4] Dilewati: Tidak ada model yang dimuat di Bagian 3.")
else:
    print(f"\n[BAGIAN 4] Dilewati: Model '{MODEL_FOR_EVALUATION}' tidak ditemukan di 'loaded_models'.")
# ==============================================================================
...
# ==============================================================================
# BAGIAN 6 (MODIFIKASI): VISUALISASI PERBANDINGAN PREDIKSI
# ==============================================================================
import random
if loaded_models and len(val_generator) > 0:
    print("\n--- Memulai Visualisasi Perbandingan Prediksi ---")

    # Ambil satu gambar secara acak untuk divisualisasikan
    sample_index = random.randint(0, len(val_generator) - 1) # Anda bisa ganti indeks ini
    print(f"Menampilkan hasil untuk sampel validasi indeks ke-{sample_index}...")

    # Ambil data asli (gambar dan ground truth)
    test_img, test_mask = val_generator[sample_index]
    test_img_single = test_img[0]
    test_mask_argmax = np.argmax(test_mask[0], axis=-1)

    # Buat prediksi untuk setiap model yang berhasil dimuat
    predictions = {}
    for model_name, model in loaded_models.items():
        print(f"Membuat prediksi dengan model: {model_name}...")
        pred = model.predict(test_img)
        predictions[model_name] = np.argmax(pred[0], axis=-1)
    ...
    # --- Tampilkan hasil perbandingan dalam grid 2x4 ---
    # Total plot = 2 (Input & GT) + 6 (Prediksi) = 8 plot
    fig, axes = plt.subplots(2, 4, figsize=(20, 10))
    plt.suptitle(f'Perbandingan Prediksi Model pada Sampel #{sample_index} (Slice {n_slice})', fontsize=20)
    
    # Flatten axtes array for easy iteration
    ax = axes.ravel()

    # Plot 1: Input Flair
    ax[0].imshow(test_img_single[:, :, n_slice, 0], cmap='gray')
    ax[0].set_title('Input: Flair', fontsize=14)
    ax[0].axis('off')

    # Plot 2: Ground Truth
    ax[1].imshow(test_mask_argmax[:, :, n_slice], cmap='viridis', vmin=0, vmax=NUM_CLASSES-1)
    ax[1].set_title('Ground Truth Mask', fontsize=14)
    ax[1].axis('off')

    # Plot 3-8: Hasil Prediksi dari setiap model
    model_names = list(loaded_models.keys())
    for i in range(len(model_names)):
        model_name = model_names[i]
        pred_mask = predictions[model_name]
        
        # Plotting starts from the 3rd subplot (index 2)
        ax[i+2].imshow(pred_mask[:, :, n_slice], cmap='viridis', vmin=0, vmax=NUM_CLASSES-1)
        ax[i+2].set_title(model_name, fontsize=12) # Gunakan nama model sebagai judul
        ax[i+2].axis('off')

    # Matikan axes yang tidak terpakai jika jumlah model < 6
    for i in range(len(model_names) + 2, 8):
        ax[i].axis('off')
