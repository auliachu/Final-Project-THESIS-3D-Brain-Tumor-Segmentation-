...
from tensorflow.keras.layers import (
    ...
    concatenate, BatchNormalization, Activation, add, multiply
)
from tensorflow.keras import backend as K

# --- BLOK 1: BLOK KONVOLUSI (Seperti pada desain Anda) ---
def conv_block_3d(input_tensor, num_filters, dropout, kernel_size=3, batch_norm=False):
    """Blok konvolusi 3D ganda dengan aktivasi ReLU."""
    kernel_initializer = 'he_normal'
    
    # Konvolusi pertama
    x = Conv3D(filters=num_filters, kernel_size=(kernel_size, kernel_size, kernel_size),
               kernel_initializer=kernel_initializer, padding='same')(input_tensor)
    if batch_norm:
...
# --- BLOK 2: GATING SIGNAL (Sesuai kode referensi) ---
def gating_signal_3d(input_tensor, out_filters, batch_norm=True):
    """Memproses sinyal dari lapisan dalam untuk digunakan sebagai gating di attention block."""
    x = Conv3D(filters=out_filters, kernel_size=(1, 1, 1), padding='same')(input_tensor)
    if batch_norm:
        x = BatchNormalization()(x)
    x = Activation('relu')(x)
    return x

# --- BLOK 3: ATTENTION BLOCK (Sesuai kode referensi) ---
def attention_block_3d(x, g, inter_filters):
    """
    Attention Block 3D.
    x: Sinyal dari skip connection (encoder).
    g: Gating signal dari lapisan yang lebih dalam.
    inter_filters: Jumlah filter intermediate.
    """
    # Theta(x)
    theta_x = Conv3D(inter_filters, kernel_size=(2, 2, 2), strides=(2, 2, 2), padding='same')(x)
    
    # Phi(g)
    phi_g = Conv3D(inter_filters, kernel_size=(1, 1, 1), padding='same')(g)
    
    # Penjumlahan dan aktivasi
    f = add([theta_x, phi_g])
    f = Activation('relu')(f)
    
    # Psi(f) untuk mendapatkan attention map
    psi_f = Conv3D(1, kernel_size=(1, 1, 1), padding='same')(f)
    alpha = Activation('sigmoid')(psi_f)
    
    # Upsample attention map agar ukurannya sama dengan x
    alpha_upsampled = UpSampling3D(size=(2, 2, 2))(alpha)
    
    # Kalikan x dengan attention map
    y = multiply([x, alpha_upsampled])
    
    return y
 
def attention_unet_3d_custom(IMG_HEIGHT, IMG_WIDTH, IMG_DEPTH, IMG_CHANNELS, num_classes):
    
    inputs = Input((IMG_HEIGHT, IMG_WIDTH, IMG_DEPTH, IMG_CHANNELS))
    
    # Encoder Path (Contraction)
    # Level 1
    c1 = conv_block_3d(inputs, 16, 0.1)
    p1 = MaxPooling3D((2, 2, 2))(c1)
    
    # Level 2
    c2 = conv_block_3d(p1, 32, 0.1)
    p2 = MaxPooling3D((2, 2, 2))(c2)

    # Level 3
    c3 = conv_block_3d(p2, 64, 0.2)
    p3 = MaxPooling3D((2, 2, 2))(c3)

    # Level 4, 0.
    c4 = conv_block_3d(p3, 128, 0.2)
    p4 = MaxPooling3D((2, 2, 2))(c4)

    # Bottleneck
    c5 = conv_block_3d(p4, 256, 0.3)

    # Decoder Path (Expansion) dengan Attention
    # Level 1 (Up dari c5 ke c4)
    g6 = gating_signal_3d(c5, 128)
    a4 = attention_block_3d(c4, g6, 128)
    u6 = Conv3DTranspose(128, (2, 2, 2), strides=(2, 2, 2), padding='same')(c5)
    u6 = concatenate([u6, a4])
    c6 = conv_block_3d(u6, 128, 0.2)

    # Level 2 (Up dari c6 ke c3)
    g7 = gating_signal_3d(c6, 64)
    a3 = attention_block_3d(c3, g7, 64)
    u7 = Conv3DTranspose(64, (2, 2, 2), strides=(2, 2, 2), padding='same')(c6)
    u7 = concatenate([u7, a3])
    c7 = conv_block_3d(u7, 64, 0.2)

    # Level 3 (Up dari c7 ke c2)
    g8 = gating_signal_3d(c7, 32)
    a2 = attention_block_3d(c2, g8, 32)
    u8 = Conv3DTranspose(32, (2, 2, 2), strides=(2, 2, 2), padding='same')(c7)
    u8 = concatenate([u8, a2])
    c8 = conv_block_3d(u8, 32, 0.1)

    # Level 4 (Up dari c8 ke c1)
    g9 = gating_signal_3d(c8, 16)
    a1 = attention_block_3d(c1, g9, 16)
    u9 = Conv3DTranspose(16, (2, 2, 2), strides=(2, 2, 2), padding='same')(c8)
    u9 = concatenate([u9, a1])
    c9 = conv_block_3d(u9, 16, 0.1)

    # Output Layer
    outputs = Conv3D(num_classes, (1, 1, 1), activation='softmax')(c9)
    model = Model(inputs=[inputs], outputs=[outputs], name="Attention_UNet_3D_Custom")
    model.summary()
...
# --- Tentukan Path untuk Dataset BraTS 2020 ---
...
OUTPUT_DIR = 'C:/Aulia/Kumpulan Eksperimen/Eksperimen 1/Model 2/'
os.makedirs(OUTPUT_DIR, exist_ok=True)
MODEL_SAVE_PATH = os.path.join(OUTPUT_DIR, 'AttnDropUnet2_best.h5')
...
model = attention_unet_3d_custom(IMG_HEIGHT, IMG_WIDTH, IMG_DEPTH, IMG_CHANNELS, NUM_CLASSES)
