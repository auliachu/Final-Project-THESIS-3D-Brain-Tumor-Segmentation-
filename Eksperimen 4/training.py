...
def attention_gate(x_skip, g_decoder, inter_filters):
    # Gerbang perhatian untuk memfokuskan model pada fitur yang relevan dari skip connection.
    phi_g = Conv3D(inter_filters, kernel_size=1, strides=1, padding='same', kernel_initializer='he_normal')(g_decoder)
    
    theta_x = Conv3D(inter_filters, kernel_size=1, strides=1, padding='same', kernel_initializer='he_normal')(x_skip)
    
    add_xg = Add()([phi_g, theta_x])
    add_xg = Activation('relu')(add_xg)
    
    psi = Conv3D(1, kernel_size=1, strides=1, padding='same', kernel_initializer='he_normal')(add_xg)
    psi = BatchNormalization()(psi)
    alpha = Activation('sigmoid')(psi)
    
    output = Multiply()([x_skip, alpha])
    return output

kernel_initializer = 'he_normal'
def attn_optimized_unet_model(input_shape, num_classes):
    ...
    attn6 = attention_gate(c4, u6, 128)
    u6 = concatenate([u6, attn6])
    ...
    attn7 = attention_gate(c3, u7, 64)
    u7 = concatenate([u7, attn7])
    ...
    attn8 = attention_gate(c2, u8, 32)
    u8 = concatenate([u8, attn8])
    ...
    attn9 = attention_gate(c1, u9, 16)
    u9 = concatenate([u9, attn9])
    ...
    model = Model(inputs=[inputs], outputs=[outputs], name="Optimized_Attn_U-Net")
    model.summary()
    return model 
...
OUTPUT_DIR = 'C:/Aulia/Kumpulan Eksperimen/Eksperimen 1/Model/'
...
model = attn_optimized_unet_model(input_shape=INPUT_SHAPE, num_classes=NUM_CLASSES)
